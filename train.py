...
# Disable PyTorch MPS on macOS to avoid "macOS 26 or later required" on older macOS (e.g. 14/15)
...
try:
    ...
    if 'macOS' in str(e) and ('2602' in str(e) or '1602' in str(e) or 'required' in str(e)):
        sys.exit(
            ...
            'Run training on CPU instead: python train.py --device cpu\n\nOriginal error: ' + str(e)
        )
    raise
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    set_time_step(time_step)  # propagate --time-step to all spiking modules
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors'), imgsz=opt.imgsz).to(device)  # create
        model2 = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors'), imgsz=opt.imgsz).to(device)  # create
        ...
        model2 = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors'), imgsz=opt.imgsz).to(device)  # create
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni - last_opt_step >= accumulate:
                ...
                torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=2.0)  # clip gradients (tighter for lightweight models)
