...
class Albumentations:
    ...
    def __init__(self, size=640):
        ...
        try:
            ...
            # Log transform names only (avoid stringifying transforms - albumentations 2.x repr can raise)
            names = [x.__class__.__name__ for x in T if getattr(x, 'p', 0) > 0]
            LOGGER.info(prefix + (', '.join(names) if names else 'transforms loaded'))
