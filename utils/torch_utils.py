...
def select_device(device='', batch_size=0, newline=True):
    ...
    if not cpu and not mps and torch.cuda.is_available():  # prefer GPU if available
        ...
        arg = 'cuda:0'
    elif mps or (not cpu and platform.system() == 'Darwin' and not torch.cuda.is_available()):
        # Prefer MPS on macOS if available; catch "macOS 26 or later required" on older macOS
        try:
            ...
            if 'macOS' in str(e) or '2602' in str(e) or '1602' in str(e):
                LOGGER.warning(f'MPS unavailable on this macOS ({e}), using CPU')
            s += 'CPU\n'
            arg = 'cpu'
    else:  # revert to CPU
        s += 'CPU\n'
