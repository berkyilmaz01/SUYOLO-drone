...
class BaseModel(nn.Module):
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            if isinstance(m, (SRepConvN, RepConvN)) and hasattr(m, 'fuse_convs'):
                m.fuse_convs()
                m.forward = m.forward_fuse  # update forward
            if isinstance(m, (SConv, Conv, DWConv)) and hasattr(m, 'bn'):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (SDDetect, Detect, DualDetect, TripleDetect, DDetect, DualDDetect, TripleDDetect, Segment, Panoptic)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None, imgsz=640):  # model, input channels, number of classes
        ...
        if isinstance(m, (SDDetect, Detect, DDetect, Segment, Panoptic)):
            ...
            #test = transf(img)  # tensor数据格式是torch(C,H,W)
            #test = test.unsqueeze(0)
            test = torch.zeros(1, ch, s, s)
            #test_list = [test for _ in range(time_step)]
            m.stride = torch.tensor([s / x.shape[-2] for x in forward(test)])  # forward
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            BasicBlock1, BasicBlock2, GhostBlock1, GhostBlock2, SGhostConv, SEncoder, SEncoderLite, SConv, TransitionBlock, Conv, AConv, ConvTranspose, Bottleneck, SPP, SPPF,
            ...
            c2 = ch[f[-1]]
        # TODO: channel, gw, gd
        elif m in {SDDetect, Detect, DualDetect, TripleDetect, DDetect, DualDDetect, TripleDDetect, Segment, Panoptic}:
